...
function weather_func(lat, lon,mountain){
    ...
    for ( var k=0; k<lat.length; k++ ) {
        var appid = "7b581f9c1783558a28dafbb3d231e5b3";
...
function changeImageColor(color) {
    ...
    image.style.filter = 'brightness(' + color + ')';
  }
  
  // 이미지 색상 변경 예시
  function updateImageColor() {
    var maxDangerousValue = Math.max.apply(null, global_dangerous);
    var minDangerousValue = Math.min.apply(null, global_dangerous);
    var currentDangerousValue = global_dangerous[0]; // 예시로 첫 번째 요소를 사용
  
    // 최대와 최소값 사이의 비율을 계산하여 0부터 1 사이의 값으로 정규화
    var normalizedValue = (currentDangerousValue - minDangerousValue) / (maxDangerousValue - minDangerousValue);
  
    // 정규화된 값에 따라 이미지의 색상을 변경
    var color = 1 - normalizedValue; // 0에 가까울수록 초록색에 가까움, 1에 가까울수록 빨간색에 가까움
...
function addMarker(position, idx) {
    ...
    if(global_dangerous[idx] < 10){
        url1= '/static/mountain_8.png'
    }else if(global_dangerous[idx] >=10 && global_dangerous[idx] <25){
        url1= '/static/mountain_7.png'
    }else if(global_dangerous[idx] >=25 && global_dangerous[idx] <40){
        url1= '/static/mountain_6.png'
    }else if(global_dangerous[idx] >=40 && global_dangerous[idx] <55){
        url1= '/static/mountain_5.png'
    }else if(global_dangerous[idx] >=55 && global_dangerous[idx] <70){
        url1= '/static/mountain_4.png'
    }else if(global_dangerous[idx] >=70 && global_dangerous[idx] <85){
        url1= '/static/mountain_3.png'
    }else if(global_dangerous[idx] >=85 && global_dangerous[idx] <95){
        url1= '/static/mountain_2.png'
    }else{
        url1= '/static/mountain_1.png'
    }
...
var content = '<style>\n' +
    ...
    '.info_overlay .title {text-align:center;padding: 5px 0 0 10px;height: 30px;background: #414967;color: white;border-bottom: 1px solid #ddd;font-size: 16px;font-weight: bold;}\n' +
    ...
    '.desc .ellipsis {overflow: hidden;text-overflow: ellipsis;white-space: nowrap; margin-top: 0.2em; color: black; font-weight: bolder;}\n'+
    '.desc .jibun {font-size: 11px;font-weight:100px;color: #888;margin-top: -2px;}\n'+
    '.info_overlay .img {margin-left: 2em;r;position: absolute;top: 6px;left: 5px; background: #59617e; width: 120px;height: 117px;border: 1px solid #ddd;color: #888;overflow: hidden;}\n'+
